...
module.exports.run = async (bot, message) => {
	...
	if (fs.existsSync(dir3 + '/assigned_role.json')) {
		const rawdata = fs.readFileSync(dir3 + '/assigned_role.json');
		const old_msg = JSON.parse(rawdata);
	...
	const embed_arr = JSON.parse(fs.readFileSync(dir3 + '/embed'));
	...
	async function delayedLog(item) {
		...
		try {
			...
			for (let index = 0; index < item.data.length; index++) {
				...
				if (item.data[index][1].custom == true) { await msg.react(`:${item.data[index][1].reaction}`); }
				else { await msg.react(`${item.data[index][1].reaction}`);}
