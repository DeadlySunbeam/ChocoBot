...
module.exports.run = async (bot, message, args) => {
  ...
  if (!args[0]) 
  {
      ...
      arr.forEach(element => {
				if(element.Role)
				canbuyitems += ` ${element.Role}${element.Emoji} -\t ${element.Price}<:Chipsik:847169552386359327> \n`;
				else
        canbuyitems += ` ${element.Name} -\t ${element.Price}<:Chipsik:847169552386359327> \n`;
      });
  ...
  var buyitem = obj['Items'].find( Item => Item.Name.toLowerCase() === search_item.toLowerCase());
  if(!buyitem) {
  ...
  var new_User = obj['Users'].find( User => User.ID === message.author.id)
	if(new_User)
	var User_Items = obj['Users'].find( User => User.ID === message.author.id)['Items']
...
async function buy() {  if (new_User)
  {
...
let request_mes = await message.channel.send(`${message.author}, Вы купили новую роль. Хотите её использовать?`);
