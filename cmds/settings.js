...
module.exports.run = async (bot, message) => {
	...
	if (words[1] == ('префикс' || '-p') && words[2] != null) {
		save(words[2]);
		message.reply('``Теперь ваш префикс ' + words[2] + '``');
	}
		...
		const msg = await message.reply('Выберите, что хотите настроить на сервере: \n1️⃣ - какие команды разрешить. \n2️⃣ - префикс для сервера');
	...
	function save(pr) {
		...
		const aw = new Discord.Collection();
