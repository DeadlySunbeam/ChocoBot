...
const bot = new Discord.Client({ intents: ['GUILDS', 'GUILD_MESSAGES', 'GUILD_MEMBERS', 'GUILD_MESSAGE_REACTIONS'] });
		...
		if (reaction.message.id === element.MESSAGE_ID) {
			...
			async function delayedLog(item) {
				...
				for (let i = 0; i < item.data.length; i++) {
					...
					if (reaction.emoji.id === item.data[i][0]) {reaction.message.guild.members.cache.get(user.id).roles.add(item.data[i][1].role);}
				}
			}
		...
		if (reaction.message.id === element.MESSAGE_ID) {
			...
			async function delayedLog(item) {
				...
				for (let i = 0; i < item.data.length; i++) {
					...
					if (reaction.emoji.id === item.data[i][0]) {reaction.message.guild.members.cache.get(user.id).roles.remove(item.data[i][1].role);}
				}
			}
